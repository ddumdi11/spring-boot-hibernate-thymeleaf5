...
public class AdminSessionSingleton {

	private static AdminSessionSingleton instance; // vor Zugriff von außen geschützt und statisch

	private AdminSessionSingleton() {
	} // privater Konstruktor mit Zugriffsschutz von außen

	public static AdminSessionSingleton getInstance() { // öffentliche Methode, Aufruf durch Code
		if (instance == null) { // nur wenn keine Instanz besteht, dann erstelle eine neue
			instance = new AdminSessionSingleton();
		}
		return instance;
	}
	...
	public Session getAdminSession() {
		...
		parameters.put(SessionParameter.USER, AlfrescoConstants.CONST_ADMIN);
		parameters.put(SessionParameter.PASSWORD, AlfrescoConstants.CONST_PWD);
